...
app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get('DATABASE_URL') or "postgresql://localhost/sabomjHW4db" # TODO 364: You should edit this to correspond to the database name YOURUNIQNAMEHW4db and create the database of that name (with whatever your uniqname is; for example, my database would be jczettaHW4db). You may also need to edit the database URL further if your computer requires a password for you to run this.
...
def get_gifs_from_giphy(search_string):
    ...
    gifs = json.loads(response.text)['data']
    return gifs
     # Replace with code
...
def get_or_create_gif(title, url):
    ...
    giffy = Gif.query.filter_by(title=title).first()
    if giffy:
        return giffy
    else:
        giffy = Gif(title=title, embedURL=url)
        db.session.add(giffy)
        db.session.commit()
        return giffy
        # Replace with code
...
def get_or_create_search_term(term):
    ...
    if search_term:
        ...
        search = get_gifs_from_giphy(term)
        for x in search:
            title = x['title']
            url = x['embed_url']
...
def search_terms():
    ...
    return render_template('search_terms.html', all_terms=all_search_terms) # Replace with code
